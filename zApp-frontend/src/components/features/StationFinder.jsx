...
const StationFinder = () => {
  ...
  const fetchStations = useCallback(async () => {
    ...
    try {
      const response = await axios.get(`http://localhost:4000/api/services/locations/services`, {
        params: { search: searchTerm },
      });
      // Apply filters directly to the fetched data
      const filtered = response.data.filter(station =>
        selectedFilters.length === 0 ||
        (station.services && selectedFilters.every(filter =>
          station.services.some(service => service.toLowerCase().includes(filter.toLowerCase()))
        ))
      );
      setStations(filtered);
    } catch (err) {
      console.error("Error fetching data:", err);
      setError("Failed to connect to the backend server. Please ensure your backend is running at http://localhost:4000.");
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    ...
    } else if (isLoaded && mapRef.current && stations.length === 0 && searchTerm) {
      // Reset to default view if no results for a search
      mapRef.current.setCenter(defaultCenter);
      mapRef.current.setZoom(5.5);
    }
  }, [stations, isLoaded, searchTerm]);
  ...
  return (
    <div className={Styles.stationFinderContainer}>
      ...
      <div className={Styles.search}>
        ...
        <button className={Styles.currentLocationButton}><img src={LocationIcons} className={Styles.locationButton}/>Use my current Location</button>
        <img src={priceToggle} className={Styles.priceToggle} alt="Show price" />
        {loading && <p>Loading results...</p>}
        {error && <div><strong>Error!</strong> <span>{error}</span></div>}
      </div>
              ...
              <div className={Styles.searchResultsContainer}>
                <div className={Styles.ResultModalTopNavContainer}>
                  <div>
                    ...
                    />
                    <img src={showResultsIcon} className={Styles.showResultsIcon} alt="Show Results" />
                  </div>
                  <img
                    src={XIconForShowResultsModal}
                    className={Styles.XIconForShowResultsModal}
                    alt="Close"
                    onClick={() => { setShowResultsModal(false); setStations([]); setSearchTerm(""); }}
                  />
                </div>
                  ...
                  stations.map((station) => (
                    <div key={station.id} className={Styles.stationCard}>
                          ...
                          {station.services
                            ...
                            .map((service, index) => (
                              <div key={index} className={Styles.serviceTagContainer}>
                                <img src={filterTag} className={Styles.filterTag} alt="Filter Tag" />
                      ...
                      <p>
                        ...
                        {station.is_open_now ? (
                          <span>
                            <img src={downArrowLightIcon} className={Styles.downArrowLightIcon} alt="Open Now" />
                          </span>
                        ) : (
                          <span>No</span>
                        )}
